...
/* Helper function for implementing the indexing strategy */
...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  std::vector<uint32_t> idx(depth, 0);
  for (int i = 0; i < out->size; i++) {
    int pos = offset;
  ...
  for (int i = 0; i < m / TILE; i++) {
    for (int j = 0; j < p / TILE; j++) {
      for (int k = 0; k < n / TILE; k++) {
        AlignedDot(&a.ptr[i * n * TILE + k * TILE * TILE],      // a[i][k]
                   &b.ptr[k * p * TILE + j * TILE * TILE],      // b[k][j]
                   &out->ptr[i * p * TILE + j * TILE * TILE]);  // c[i][j]
      }
    }
  }
  /// END SOLUTION

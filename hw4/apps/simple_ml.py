...
def softmax_loss(Z, y_one_hot):
    ...
    """
    ### BEGIN YOUR SOLUTION
    return Z.exp().sum(axis=1).log().sum() - (y_one_hot * Z).sum() / Z.shape[0]
    ### END YOUR SOLUTION
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(0, X.shape[0], batch):
        ...
        y_one_hot = np.zeros((batch, logits.shape[-1]))
        y_one_hot[np.arange(batch), y_batch] = 1
        y_one_hot = ndl.Tensor(y_one_hot)
        loss = softmax_loss(logits, y_one_hot)
        loss.backward()
        W1 = (W1 - lr * W1.grad).detach()   #! detach() is used to avoid the accumulation of gradients

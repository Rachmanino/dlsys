...
__device__ size_t index2loc(size_t i, CudaVec shape, CudaVec strides, size_t offset) {
  size_t loc = offset;
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    size_t loc = index2loc(gid, shape, strides, offset);
    out[gid] = a[loc];
  }
  /// END SOLUTION
}
...
void EwiseSetitem(const CudaArray& a, CudaArray* out, std::vector<int32_t> shape,
   ...
   */
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size); //! 这里不能用out.size, out和a的size不一样
...
// Elementwise and scalar operations
////////////////////////////////////////////////////////////////////////////////
__global__ void Naive_MatMulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N, uint32_t P) {
  /**
   * Kernel to multiply two matrices in a naive way.
   * 
   * Args:
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  Naive_MatMulKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, M, N, P);
  /// END SOLUTION
}
...
////////////////////////////////////////////////////////////////////////////////
enum ReduceType { REDUCE_MAX, REDUCE_SUM };
__global__ void ReduceKernel(const scalar_t* a, scalar_t* out, size_t out_size, size_t reduce_size, 
                             ReduceType type) {
  /**
   * Kernel to reduce by taking maximum or sum over contiguous blocks.
   */
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < out_size) {
    switch (type) {
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  ReduceKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, reduce_size, REDUCE_MAX);
  /// END SOLUTION
}
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  ReduceKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, reduce_size, REDUCE_SUM);
  /// END SOLUTION
}
